...
const FALLBACK_PORT = parseInt(process.env.FALLBACK_PORT || '3008');
...
// Import bridges
import neonBridge from './neon-mcp-bridge.js';
import AppStoreConnectBridge from './app-store-connect-bridge.js';
...
async function startServers() {
  ...
  primaryServer.listen(PRIMARY_PORT, '0.0.0.0', () => {
    logger.info(`🌐 MCP Unified Gateway PRIMARY running on port ${PRIMARY_PORT}`);
  ...
  // Start fallback server (3008)
  fallbackServer.listen(FALLBACK_PORT, '0.0.0.0', () => {
    logger.info(`🔄 MCP Unified Gateway FALLBACK running on port ${FALLBACK_PORT}`);
