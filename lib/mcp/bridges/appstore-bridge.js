...
class AppStoreConnectBridge {
  constructor(envVars = null) {
    ...
    // Initialize from provided environment variables or process.env
    this.initializeFromEnvironment(envVars);
  }

  initializeFromEnvironment(envVars = null) {
    try {
      // Use provided env vars or fall back to process.env
      const env = envVars || process.env;
      
      this.keyId = env.APPLE_KEY_ID;
      this.issuerId = env.APPLE_ISSUER_ID;
      this.privateKey = env.APPLE_PRIVATE_KEY;
      this.apiKey = this.privateKey; // Use private key as API key for JWT generation
