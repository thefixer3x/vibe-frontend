...
class NeonMCPBridge {
  constructor() {
    ...
    this.pool = null;
    // Try NEON_CONNECTION_STRING first, then fallback to DATABASE_URL or POSTGRES_URL
  ...
  async connectToDatabase() {
    // Skip database connection if no connection string is provided or if it's a placeholder
    if (!this.connectionString || 
        this.connectionString.includes('user:password@host.neon.tech') ||
        this.connectionString.includes('user:password@host')) {
      logger.warn('⚠️  Neon connection string not configured. Database features will be unavailable, but API features will work.');
      this.isConnected = false;
      return;
    }
