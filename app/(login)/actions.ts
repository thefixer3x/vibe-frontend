...
export const signIn = validatedAction(signInSchema, async (data, formData) => {
  ...
  if (redirectTo === 'checkout') {
    ...
    const checkoutUrl = await createCheckoutSession({ team: foundTeam, priceId });
...
export const signUp = validatedAction(signUpSchema, async (data, formData) => {
  ...
  // Create user with admin role for single user setup
  ...
  // Create default team for single user
  const newTeam: NewTeam = {
    name: `${createdUser.name || email}'s Workspace`,
    planName: 'pro', // Give yourself pro plan
  ...
  if (!createdTeam) {
    // If team creation fails, clean up user
